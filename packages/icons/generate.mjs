...
function kotlinElement(element, index, viewBox, indent) {
  const inner = `${indent}    `;
  const path =
    element.kind === 'path'
      ? `val path${index} = Path().apply {
${element.fillRule === 'evenodd' ? `${inner}fillType = PathFillType.EvenOdd\n` : ''}
${element.commands.map((command) => `${inner}${kotlinPathCommand(command, viewBox)}`).join('\n')}
${indent}}`
      : null;
  const target =
    element.kind === 'circle'
...
function generateAndroid(icons) {
  ...
  // 아이콘마다 함수를 따로 뽑는다. 그리기 코드를 전부 Canvas 람다 하나에 넣으면 JVM 의 메서드당
  // 64KB 바이트코드 한계를 넘겨 Kotlin 컴파일러가 MethodTooLargeException 으로 죽는다.
  const drawFunctions = icons
    .map(
      (icon) => `private fun DrawScope.draw${icon.type}() {
    ${icon.elements.map((element, index) => kotlinElement(element, index, icon.viewBox, '    ')).join('\n    ')}
}`,
    )
    .join('\n\n');
  const switches = icons
    .map((icon) => `NookIconName.${icon.type} -> draw${icon.type}()`)
    .join('\n            ');
  return `// Generated by packages/icons/generate.mjs. Do not edit directly.
