...
/**
 * Figma `List/Popup_Archive > Property 1=Default | selected`.
 * 아카이브를 고르는 행 — 좌측 색 스와치 + 이름, 우측 체크 표시.
...
function ArchiveSelectRow({
  archive,
  selected,
  onSelectedChange,
  className,
}: ArchiveSelectRowProps) {
  return (
    <label
      className={cn(
    ...
    >
      ...
      <span className="flex min-w-0 items-center gap-4">
        {/* 시안의 스와치는 10px 정사각 (ArchiveTag 의 8px 과 다르다) */}
        <span
          className={cn('size-2.5 shrink-0', COLOR_BG_CLASS[archive.color])}
          aria-hidden="true"
        />
