 ...
 * 검색 필드(뒤로가기·입력·지우기) + 건수 + 장소 카드 그리드.
 *
 * 시안의 아카이브 칩 필터는 서버 검색 API 가 그룹 필터를 아직 지원하지 않아 뺐다 —
 * `groupId` 파라미터가 생기면 되살린다(UI 는 PR #95 에 있다).
 *
...
export function PlaceSearchPanel({
  ...
  const page = useSearchSavedPlaces(debouncedQuery);
