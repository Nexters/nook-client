...
interface PinnedHeaderLayoutProps {
  /** 상단에 고정할 영역. 보통 `<Header />` 하나지만, 그룹 상세처럼 정보 블록까지 함께 붙기도 한다. */
  header: ReactNode;
  children: ReactNode;
...
/**
 * 헤더는 화면에 고정하고 콘텐츠만 스크롤되는 화면(그룹 상세·게시물 상세·최상위 탭)의 공용 레이아웃.
 *
 ...
 *
 * 고정 영역 높이는 화면마다(그리고 그룹 이름 줄수처럼 데이터마다) 달라서 실측해 콘텐츠
...
export function PinnedHeaderLayout({
  ...
  // 데이터가 늦게 도착해 헤더가 길어지는 경우(그룹 상세의 작성자 줄)는 렌더마다 다시 재서
