...
> [배포 빌드 설치 가이드](<배포_빌드_설치_가이드.md>)와
> [iOS EAS 빌드 및 App Store 제출](<iOS_EAS_빌드_및_App_Store_제출.md>) 참고.
...
- 로컬 웹을 앱 WebView에 띄우는 법(포트 프록시, LAN IP·네이버 지도 SDK 제약 등)은
  [배포 빌드 설치 가이드의 "로컬 웹을 앱에 띄울 때" 절](<배포_빌드_설치_가이드.md>)을
  그대로 따른다 — 로컬 빌드든 EAS dev-client 빌드든 동일하다.

## 5. 이 문서가 다루지 않는 것

- EAS 클라우드로 서명된 dev-client/배포용 빌드 설치, 새 테스트 기기 등록:
  [배포 빌드 설치 가이드](<배포_빌드_설치_가이드.md>)
- production 빌드와 App Store Connect 제출:
  [iOS EAS 빌드 및 App Store 제출](<iOS_EAS_빌드_및_App_Store_제출.md>)
