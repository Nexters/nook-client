...
| 작성자 | 작성일 | 수정일 | 관련 작업 |
| --- | --- | --- | --- |
| coldbrow | 2026-07-27 | 2026-07-27 | NOOK-12 |
...
- 도형은 `path`, `circle`, `rect`를 지원한다. `line`과 `polygon`은 path로 변환한다.
- `rect`의 모서리는 `rx`와 `ry`가 같아야 한다. 세 플랫폼 모두 반지름 하나로만 그리기 때문이다
  (SwiftUI `cornerRadius`, Compose `CornerRadius`).
- 경로 명령은 절대 좌표 `M`, `L`, `H`, `V`, `C`, `Q`, `Z`를 지원한다.
- `transform`은 지원하지 않는다. 회전·이동은 좌표에 미리 반영해서 내보낸다.
- `<defs>` 안(clipPath·mask 정의)은 그리지 않는다.
