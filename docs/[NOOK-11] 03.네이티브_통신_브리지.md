...
- 모든 메시지는 `{ v, type, payload }` 형태. 공통 계약 패키지의 파서가 버전·타입·payload 를 런타임에도 검증한다.
...
- web·mobile 은 공통 계약의 타입과 런타임 파서를 함께 사용한다. 지원하지 않는 버전이나 잘못된 payload 는 양쪽 모두 무시한다.

## 코드 위치

- `packages/bridge-contracts/src/` — 메시지 계약과 런타임 파서: `message` / `native-to-web` / `web-to-native` / `parse`
- `apps/web/src/native-bridge/` — 웹측 클라이언트
- `apps/mobile/src/bridge/useWebViewBridge.ts` — mobile 브리지 처리와 네이티브 명령 실행
- `apps/mobile/src/webview/navigationPolicy.ts` — WebView URL 허용 정책
- `apps/mobile/App.tsx` — Expo 셸 렌더링
